...
# * 2.0.0   29-Jul-2016 Change to use for P1.5
# * 2.0.1   3-Aug-2016 Update command open camera, resolution. Remove all
#				scripts related to CPLD
#*******************************************************************************
...
parser.add_argument("-o", "--open", help="open or close camera",
...
parser.add_argument("-g", "--gain", help="select gain value",
					action="store", choices=['100','200','400','800','1600','2400','3200','4200','6100'])
...
if args.camera:
	...
	if args.open:
		...
		byte_count = 5
		for camera in args.camera:
			fstring += open_byte(args.open)
			fstring += " "
			byte_count += 1
		open_string = "adb shell \"echo " + str(byte_count) + " 0x0000 0x00 0x00 " + m_bitmask + " "\
		+ fstring + " > /sys/class/i2c-adapter/i2c-11/i2c-0/0-0010/i2c_w\""
		print open_string
		execute(open_string)
		print ""
		if args.ucid:
			ucid_string = "adb shell 'echo 4 0x0000 0x00 0x10 " + ucid_to_hex(args.ucid) + " > /sys/class/i2c-adapter/i2c-11/i2c-0/0-0010/i2c_w'"
	...
	if args.stream:
		...
		camera_string = "adb shell \"echo " + str(byte_count) + " 0x0000 0x02 0x00 " + m_bitmask \
						+ " " + stream_string + "> /sys/class/i2c-adapter/i2c-11/i2c-0/0-0010/i2c_w\""
		print camera_string
...
if args.ucid:
	if not args.camera:
		# Light active ucid
		str = "adb shell 'echo 4 0x0000 0x00 0x10 " + ucid_to_hex(args.ucid) + " > /sys/class/i2c-adapter/i2c-11/i2c-0/0-0010/i2c_w'"
...
if args.focus:
	...
	if args.read:
		focus_string = "adb shell \"echo " + str(byte_count) + " 0x0000 0x48 0x00 " + m_bitmask + " "\
		...
		for camera in args.camera:
			if camera == 'g':
				...
				byte_count +=4
		read_string = "adb shell \"echo " + str(byte_count) + " 0x0048 "+"> /sys/class/i2c-adapter/i2c-11/i2c-0/0-0010/i2c_br\""
		print read_string
		execute(read_string)
		print "adb shell \"cat /sys/class/i2c-adapter/i2c-11/i2c-0/0-0010/i2c_br\""
		execute("adb shell \"cat /sys/class/i2c-adapter/i2c-11/i2c-0/0-0010/i2c_br\n\"")
		print ""
	else:
		for camera in args.camera:
...
if args.exposure:
	if args.camera:
		if args.read:
			if args.ucid:
				ucid = ucid_to_hex(args.ucid)
				exposure_string = "adb shell \"echo 7" + " 0x0000 0x32 0x00 " + m_bitmask + " " + ucid +\
					" > /sys/class/i2c-adapter/i2c-11/i2c-0/0-0010/i2c_w\""
				...
				for camera in args.camera:
					if camera == 'g':
						...
						byte_count +=8
				read_string = "adb shell \"echo " + str(byte_count) + " 0x0032 "+"> /sys/class/i2c-adapter/i2c-11/i2c-0/0-0010/i2c_br\""
				print read_string
				execute(read_string)
				print "adb shell \"cat /sys/class/i2c-adapter/i2c-11/i2c-0/0-0010/i2c_br\""
				execute("adb shell \"cat /sys/class/i2c-adapter/i2c-11/i2c-0/0-0010/i2c_br\n\"")
			...
			if args.ucid:
				...
				ucid + " " + fstring + " > /sys/class/i2c-adapter/i2c-11/i2c-0/0-0010/i2c_w\""
			else:
...
if args.gain:
	if args.camera:
		if args.read:
			if args.ucid:
				ucid = ucid_to_hex(args.ucid)
				gain_string = "adb shell \"echo 7" + " 0x0000 0x30 0x00 " + m_bitmask + " " + ucid +\
					" > /sys/class/i2c-adapter/i2c-11/i2c-0/0-0010/i2c_w\""
				print gain_string
				execute(gain_string)
				byte_count = 0
				for camera in args.camera:
					if camera == 'g':
						...
						byte_count +=4
				read_string = "adb shell \"echo " + str(byte_count) + " 0x0030 "+"> /sys/class/i2c-adapter/i2c-11/i2c-0/0-0010/i2c_br\""
				print read_string
				execute(read_string)
				print "adb shell \"cat /sys/class/i2c-adapter/i2c-11/i2c-0/0-0010/i2c_br\""
				execute("adb shell \"cat /sys/class/i2c-adapter/i2c-11/i2c-0/0-0010/i2c_br\n\"")
			...
			for camera in args.camera:
				# add the argument n number of times for n cameras
				fstring += change_iso_value(args.gain)
				fstring += " 0x00 0x00 "
				byte_count += 4
			print "ISO is " + args.gain
			if args.ucid:
				ucid = ucid_to_hex(args.ucid)
...
if args.fps:
	if args.camera:
		if args.read:
			if args.ucid:
				ucid = ucid_to_hex(args.ucid)
				fps_string = "adb shell \"echo 7" + " 0x0000 0x50 0x00 " + m_bitmask + " " + ucid +\
					" > /sys/class/i2c-adapter/i2c-11/i2c-0/0-0010/i2c_w\""
				print fps_string
				execute(fps_string)
				byte_count = 0
				for camera in args.camera:
					if camera == 'g':
						...
						byte_count +=2
				read_string = "adb shell \"echo " + str(byte_count) + " 0x0050 "+"> /sys/class/i2c-adapter/i2c-11/i2c-0/0-0010/i2c_br\""
				print read_string
				execute(read_string)
				print "adb shell \"cat /sys/class/i2c-adapter/i2c-11/i2c-0/0-0010/i2c_br\""
				execute("adb shell \"cat /sys/class/i2c-adapter/i2c-11/i2c-0/0-0010/i2c_br\n\"")
			...
			if args.ucid:
				ucid = ucid_to_hex(args.ucid)
				fps_string = "adb shell \"echo " + str(byte_count + 2) + " 0x0000 0x50 0x00 " + m_bitmask + " " + ucid + " "\
				+ fstring + \
				" > /sys/class/i2c-adapter/i2c-11/i2c-0/0-0010/i2c_w\""
			else:
				fps_string = "adb shell \"echo " + str(byte_count) + " 0x0000 0x50 0x00 " + m_bitmask + " "\
				+ fstring + \
				" > /sys/class/i2c-adapter/i2c-11/i2c-0/0-0010/i2c_w\""
			print fps_string
			execute(fps_string)
...
if args.resolution:
	if args.camera:
		if args.read:
			if args.ucid:
				ucid = ucid_to_hex(args.ucid)
				resolution_string = "adb shell \"echo 7" + " 0x0000 0x2c 0x00 " + m_bitmask + " " + ucid +\
					" > /sys/class/i2c-adapter/i2c-11/i2c-0/0-0010/i2c_w\""
				...
				for camera in args.camera:
					if camera == 'g':
						...
						byte_count +=8
				read_string = "adb shell \"echo " + str(byte_count) + " 0x002c "+"> /sys/class/i2c-adapter/i2c-11/i2c-0/0-0010/i2c_br\""
				print read_string
				execute(read_string)
				print "adb shell \"cat /sys/class/i2c-adapter/i2c-11/i2c-0/0-0010/i2c_br\""
				execute("adb shell \"cat /sys/class/i2c-adapter/i2c-11/i2c-0/0-0010/i2c_br\n\"")
			...
			if args.ucid:
				ucid = ucid_to_hex(args.ucid)
				resolution_string = "adb shell \"echo " + str(byte_count + 2) + " 0x0000 0x2C 0x00 " + m_bitmask + " " + \
				ucid + " " + fstring + " > /sys/class/i2c-adapter/i2c-11/i2c-0/0-0010/i2c_w\""
			else:
				resolution_string = "adb shell \"echo " + str(byte_count) + " 0x0000 0x2C 0x00 " + m_bitmask + " " + \
				fstring + " > /sys/class/i2c-adapter/i2c-11/i2c-0/0-0010/i2c_w\""
			print resolution_string
			execute(resolution_string)
			print ""
...
if args.read_hall_sensor_value:
	...
	if args.lens:
		...
		output_string = 'adb shell \"echo 5 0x0000 0x40 0x00 ' +\
						m_bitmask + ' > /sys/class/i2c-adapter/i2c-11/i2c-0/0-0010/i2c_w\"'
		print output_string
	...
	if args.mirror:
		...
		output_string = "adb shell \"echo 5 0x0000 0x44 0x00 " + m_bitmask + " > /sys/class/i2c-adapter/i2c-11/i2c-0/0-0010/i2c_w\""
		print output_string
		execute(output_string)
...
if args.go_to_hall_sensor_value:
	...
	if args.lens:
		...
		destination_string + " > /sys/class/i2c-adapter/i2c-11/i2c-0/0-0010/i2c_w\""
		print output_string
		execute(output_string)
