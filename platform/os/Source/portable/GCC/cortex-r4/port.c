...
/*
 * Setup the system tick for FreeRTOS using Timer 1
 */
static void vPortSetupTickInterrupt( void );

/*
 * Update the RTC timer
 */
...
static void vPortSetupTickInterrupt( void )
{
	hal_timer_t tick =
	{
		.chid = FREERTOS_SYSTEM_TICK,
		.period = 1000,
		.callback_handler = vPortTickISR
	};
	/* Configure the timer 1 has period is 1ms. */
	hal_timer_init(&tick);
	hal_timer_start(tick.chid);
}
